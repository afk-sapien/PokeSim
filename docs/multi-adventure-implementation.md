...
The library supports ROM upload and reuse, adventure creation, start, save and stop, archive and restore, profile settings, a running-game limit, automatic useful exchanges across all eligible running adventures, backups, and legacy import. Existing game pages use adventure-scoped routes and an adventure switcher. The Library opens directly with a keyless browser session and protects writes against cross-site requests. The default bind is local-only. Remote access requires a trusted private network or authentication at the reverse proxy. All private worker routes retain their own authentication.
...
Simulation pace is now one durable Library setting, defaulting to the recommended 1×. Live screens and adventure settings no longer change it. Running workers acknowledge updates through their private interface, reconnecting workers retry, and new or restarted adventures inherit the global pace. New Cable Club sessions use the same setting. The three smoke games are intentionally set to Max for testing. See the [global pace validation and measured throughput](validation/global-pace-20260915.json).
