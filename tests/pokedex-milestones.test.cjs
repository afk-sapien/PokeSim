...
test('new milestones refresh cached cards without other inventory changes', async () => {
  ...
  assert.doesNotMatch(ui.element('#grid').innerHTML, /level 100 star earned/)
  ...
  assert.match(ui.element('#grid').innerHTML, /level 100 star earned/)
...
test('three-star counts separate live partners from historical species and refresh filters', async () => {
  const ui = view()
  await ui.update({})
  assert.equal(ui.element('#sum-three-star').textContent, '0')
  await ui.update({party: [{dex: 1, dv_stars: 3}],
    milestones: {three_star_held: 1, high_quality_species: [1, 2], perfect_species: [3]}})
  assert.equal(ui.element('#sum-three-star').textContent, '1')
  assert.equal(ui.element('#sum-quality').innerHTML, '3 <small>/ 151</small>')
  ui.filter('quality')
