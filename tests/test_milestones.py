...
@pytest.mark.parametrize('dvs, stars, total', [
    ((0, 0, 0, 0, 0), 1, 0),
    ((7, 7, 7, 8, 8), 1, 37),
    ((8, 7, 7, 8, 8), 2, 38),
    ((11, 12, 12, 12, 12), 2, 59),
...
def test_dv_star_boundaries(dvs, stars, total):
    from pokesim.pokemon import dv_rating
    assert dv_rating({'dvs': dvs, 'level': 5, 'stat_exp': (0,) * 5}) == {
        'dv_stars': stars, 'dv_total': total, 'dv_percent': round(total * 100 / 75, 1)}
    assert dv_rating({'dvs': list(dvs), 'level': 100, 'stat_exp': (65535,) * 5})['dv_stars'] == stars


@pytest.mark.parametrize('dvs', [None, (), (15,) * 4, (15,) * 6, (True,) * 5,
                               (15.0,) * 5, ('15',) * 5, (16,) * 5, (-1,) * 5])
def test_unknown_dvs_do_not_receive_stars(dvs):
    from pokesim.pokemon import dv_rating
    assert dv_rating({'dvs': dvs}) == {'dv_stars': None, 'dv_total': None, 'dv_percent': None}


def test_three_star_history_is_durable_but_held_count_tracks_inventory(tracking):
    store, tracker = tracking
    strong = replace(partner(1, 20), dvs=(12,) * 5)
    boxed = stored(0, species=species(2), dvs=(13,) * 5, trainer_id=123)
    s = snapshot([boxed], party=(strong, strong, partner(3, 20, True)))
    tracker.observe(s)
    assert status(store)['high_quality_species'] == []
    settle(tracker, s)
    value = apply(live_status(s.to_dict()), store)
    assert value['milestones']['three_star_held'] == 3
    assert value['milestones']['perfect_held'] == 1
    assert value['milestones']['high_quality_species'] == [1, 2, 3]
    assert [p['dv_stars'] for p in value['party']] == [3, 3, 4]
    assert value['storage']['pokemon'][0]['dv_stars'] == 3
    tracker = MilestoneTracker(store)
    s = snap(party=(partner(25, 5),))
    settle(tracker, s)
    value = apply(live_status(s.to_dict()), store)['milestones']
    assert value['three_star_held'] == 0
    assert value['high_quality_species'] == [1, 2, 3]
    tracker.reset()
    assert status(store)['high_quality_species'] == []


def test_older_milestones_backfill_only_known_perfect_species(tracking):
    store, tracker = tracking
    store.set(KEY, {'level_100': [1, 2, 3], 'perfect_species': [3],
                    'perfect_groups': {'123:153': 1}, 'perfect_catches': 0})
    assert status(store)['high_quality_species'] == [3]
    settle(tracker, snap(party=(replace(partner(25, 20), dvs=(12,) * 5),)))
    assert status(store)['high_quality_species'] == [3, 25]
    assert status(store)['perfect_found'] == 1


def test_three_star_species_credit_requires_stable_valid_observations(tracking):
    store, tracker = tracking
    s = snap(party=(replace(partner(134, 20), dvs=(12,) * 5),))
    tracker.observe(s)
    tracker.observe(snap())
    assert status(store)['high_quality_species'] == []
    store.set('trade_hold', True)
