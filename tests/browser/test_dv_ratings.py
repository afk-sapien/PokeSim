...
def test_dv_appraisals_across_pc_and_pokedex(page, game, width):
    ...
    expect(page.locator('#sum-three-star')).to_have_text('1')
    expect(page.locator('#sum-quality')).to_have_text('1 / 151')
    ...
    assert page.evaluate('document.documentElement.scrollWidth <= window.innerWidth')
    page.locator('.dex-count-help summary').click()
    expect(page.get_by_role('link', name='Browse your 3★ and 4★ partners')).to_be_visible()
    page.screenshot(path=f'/tmp/pokesim-stars-dex-{width}.png', full_page=True)
    page.get_by_role('link', name='Browse your 3★ and 4★ partners').click()
    expect(page.locator('.pc-mon')).to_have_count(2)
    expect(page.locator('#pc-rating')).to_have_value('3plus')
