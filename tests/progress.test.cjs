...
test('league wins scale to their own best and a rewind shows as a drop', () => {
  const {Progress} = load()
  const rows = [{ts: 0, owned: 10, badges: 2, league: 0}, {ts: 5, owned: 9, badges: 2, league: 4}]
  const [owned, badges, league] = Progress.describe(rows, 10)
  assert.deepEqual([owned.first, owned.last, owned.max], [10, 9, 151])
  assert.deepEqual([badges.first, badges.last, badges.max], [2, 2, 8])
