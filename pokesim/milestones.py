...
def empty():
    return {'level_100': [], 'perfect_species': [], 'perfect_groups': {}, 'perfect_catches': 0}


def status(store):
    value = store.get(KEY) or empty()
...
def apply(payload, store):
    ...
    party = [dict(mon, perfect_dvs=is_perfect(mon)) for mon in payload.get('party', ())]
    storage = payload.get('storage')
    boxed = [dict(mon, perfect_dvs=is_perfect(mon)) for mon in (storage or {}).get('pokemon', ())]
