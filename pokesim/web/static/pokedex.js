...
function milestoneBadges(dex) {
  return `<span class="milestone-badges">${maxed.has(dex) ? '<span class="mastery-badge"><span aria-hidden="true">★</span> Lv. 100</span>' : ''}${perfectSpecies.has(dex) ? '<span class="perfect-badge"><span aria-hidden="true">✦</span> Perfect DV</span>' : ''}</span>`
}
...
function renderGrid() {
  ...
  const signature = JSON.stringify([rows, [...owned], [...seen], [...held], [...plan], hunting, catches, [...maxed], [...perfectSpecies]])
  ...
  $('#grid').innerHTML = rows.map((entry) => {
    ...
    return `<button class="dex-card ${state}${perfectSpecies.has(entry.dex) ? ' perfect-entry' : ''}" data-dex="${entry.dex}" aria-label="${esc(entry.name)}, number ${num(entry.dex)}, ${RECORD_LABELS[state]}${maxed.has(entry.dex) ? ', level 100 star earned' : ''}${perfectSpecies.has(entry.dex) ? ', perfect DV species found' : ''}, ${counts}" aria-describedby="catch-tracking-note">
...
function renderDetail(dex, refresh = false) {
  ...
  const signature = JSON.stringify([entry, state, project, copies, hunting, catches, maxed.has(dex), perfectSpecies.has(dex)])
